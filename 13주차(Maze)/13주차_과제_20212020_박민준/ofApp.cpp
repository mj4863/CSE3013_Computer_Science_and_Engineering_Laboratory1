...
bool ofApp::BFS() // BFS탐색을 하는 함수
{
	...
	while (!q.empty()) {
		...
		if (x == endX && y == endY) {
			...
			while (current != pair<int, int>{startX, startY}) {
				bfsShortestPath.push_back(current);
				current = parent[current.first][current.second];
			}
			bfsShortestPath.push_back({ startX, startY });
			reverse(bfsShortestPath.begin(), bfsShortestPath.end());
			...
			while (!q.empty()) {
				...
				for (int dir = 0; dir < 4; dir++) {
					...
					if (nx > 0 && nx < HEIGHT && ny > 0 && ny < WIDTH && maze[nx][ny] == ' ' && !visited[nx][ny]) {
						...
						bfsFullPath.push_back({ {x, y}, {nx, ny} });
					}
				}
			}
...
void ofApp::bfsdraw()
{
	...
	ofSetColor(255, 0, 0);
	ofSetLineWidth(2);
	for (const auto& edge : bfsFullPath) {
		int x1 = edge.first.second * 30;
		int y1 = edge.first.first * 30;
		int x2 = edge.second.second * 30;
		int y2 = edge.second.first * 30;
		ofDrawLine(x1, y1, x2, y2);
	}
	...
	ofSetColor(0, 0, 255);
	ofSetLineWidth(4);
	for (size_t i = 1; i < bfsShortestPath.size(); i++) {
		int x1 = bfsShortestPath[i - 1].second * 30;
		int y1 = bfsShortestPath[i - 1].first * 30;
		int x2 = bfsShortestPath[i].second * 30;
		int y2 = bfsShortestPath[i].first * 30;
		ofDrawLine(x1, y1, x2, y2);
	}
}
